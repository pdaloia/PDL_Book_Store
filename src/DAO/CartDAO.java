...
public class CartDAO {
	...
	public CartDAO() throws ClassNotFoundException {
        ...
        public List<CartBean> getCartByUser(String username) throws SQLException{
            ...
            while(r.next()){
                CartBean bean = new CartBean(r.getString("username"), r.getString("bid"), r.getString("title"), r.getFloat("price"), r.getInt("quantity"));
        ...
        public CartBean addProductToCart(String bid,String title, String username, float price, int quantity){
            ...
            try{
              ...
              Connection con = this.ds.getConnection();
              PreparedStatement stmt = con.prepareStatement("INSERT INTO cart (username,bid,title,price,quantity) values(?,?,?,?,?)");
        ...
        public void updateCartProduct(String bid, String username, float price, int quantity, boolean increament){
            ...
            try{
              ...
              if(increament){
                 ...
                 PreparedStatement stmt = con.prepareStatement("UPDATE cart set price=?, quantity=? where bid=? and username=? "); 
        ...
        public CartBean getCartProductByProductId(String bid, String username){
            try{
                ...
                Connection con = this.ds.getConnection();
                PreparedStatement p = con.prepareStatement("SELECT * from Cart where TRIM(username) like ? and TRIM(bid) like ?");
                p.setString(1, username.trim());
                p.setString(2, bid.trim());
                ResultSet r = p.executeQuery();
                CartBean bean=null;
                while (r.next()) {
                    bean = new CartBean(r.getString("username"), r.getString("bid"), r.getString("title"), r.getFloat("price"), r.getInt("quantity"));
